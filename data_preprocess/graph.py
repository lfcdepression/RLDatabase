...
def graph_constructor_dir(data_path,save_path,save_name,cutoff=6.0):
            ...
            if pocket_features.shape[0] !=0:
                if pocket_coords.shape[0] !=0:
                    ...
                    for i in range(len(name_list)):
                        name = name_list[i]
                        if name_list[i] == dir_name:
                            indicator = np.ones((final_features.shape[0], 1)) * (i + 1)
                            indicator = indicator.astype(int)
                    # 保存指示器、节点标签、节点属性和图标签到文件
                    with open(os.path.join(save_path, f"{save_name}_graph_indicator.txt"),'ab') as f:
                        np.savetxt(f, indicator, fmt='%s', delimiter=', ')
                    f.close()

                    with open(os.path.join(save_path, f"{save_name}_node_labels.txt"),'ab') as f:
                        np.savetxt(f, final_features, fmt='%.4f', delimiter=', ')
                    f.close()

                    with open(os.path.join(save_path, f"{save_name}_node_attributes.txt"),'ab') as f:
...
def main():
    graph_constructor_dir(data_path='PDBind_without_pdb/core',save_path='PDBgraph_files',save_name='test',cutoff=6.0)

    graph_constructor_dir(data_path='PDBind_without_pdb/refined',save_path='PDBgraph_files',save_name='train_val',cutoff=6.0)
